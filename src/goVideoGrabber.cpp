...
//--------------------------------------------------------------------
void goVideoGrabber::listDevices(){
		...
		*/
		...
		for(int i = 0 ; i < (*deviceList)->count ; ++i)
		{
			...
			if(nameRec.flags != sgDeviceNameFlagDeviceUnavailable){
				...
				deviceVec.push_back((string)p2cstr(pascalName));
				deviceCount++;
			}
		}
		printf( "-------------------------------------\n");
...
//--------------------------------------------------------------------
void goVideoGrabber::setVerbose(bool bTalkToMe){
	bVerbose = bTalkToMe;
...
#ifdef OF_VIDEO_CAPTURE_QUICKTIME
//---------------------------------
void goVideoGrabber::setDeviceID(string _deviceID){
	deviceID = -1;
	for (int i = 0; i < deviceVec.size(); i++) {
		if (_deviceID == deviceVec[i]) {
			deviceID = i;
			break;
		}
	}
...
//--------------------------------------------------------------------
void goVideoGrabber::setDeviceID(int _deviceID){
	deviceID		= _deviceID;
	bChooseDevice	= true;
}

//--------------------------------------------------------------------
void goVideoGrabber::setDesiredFrameRate(int framerate){
	attemptFramerate = framerate;
}

//---------------------------------------------------------------------------
unsigned char * goVideoGrabber::getPixels(){
...
ofTexture & goVideoGrabber::getTextureReference(){
	if(!tex.bAllocated() ){
		ofLog(OF_LOG_WARNING, "goVideoGrabber - getTextureReference - texture is not allocated");
	}
	return tex;
}

//---------------------------------------------------------------------------
bool  goVideoGrabber::isFrameNew(){
	return bIsFrameNew;
}

//--------------------------------------------------------------------
void goVideoGrabber::update(){
	grabFrame();
}

//--------------------------------------------------------------------
void goVideoGrabber::grabFrame(){
...
//--------------------------------------------------------------------
void goVideoGrabber::videoSettings(void){
...
//---------------------------------------------------------------------
bool goVideoGrabber::saveSettings(){
...
//---------------------------------------------------------------------
bool goVideoGrabber::loadSettings(){
...
//------------------------------------------------------
bool goVideoGrabber::qtInitSeqGrabber(){
...
//--------------------------------------------------------------------
bool goVideoGrabber::qtCloseSeqGrabber(){
...
//--------------------------------------------------------------------
bool goVideoGrabber::qtSelectDevice(int deviceNumber, bool didWeChooseADevice){
...
//--------------------------------------------------------------------
bool goVideoGrabber::initGrabber(int w, int h, bool setUseTexture){
	...
	//---------------------------------
		...
		bail:
			...
			ofLog(OF_LOG_ERROR, "***** goVideoGrabber error *****");
			ofLog(OF_LOG_ERROR, "-------------------------------------\n");
	...
	//--------------------------------
		...
		if(gstUtils.initGrabber(w,h,attemptFramerate)){
			...
			bGrabberInited = true;
			ofLog(OF_LOG_VERBOSE, "goVideoGrabber: initied");
		}else{
			bGrabberInited = false;
			ofLog(OF_LOG_ERROR, "goVideoGrabber: couldn't init");
		}
		return bGrabberInited;
...
//----------------------------------------------------------
void goVideoGrabber::setAnchorPercent(float xPct, float yPct){
    if (bUseTexture)tex.setAnchorPercent(xPct, yPct);
}

//----------------------------------------------------------
void goVideoGrabber::setAnchorPoint(int x, int y){
    if (bUseTexture)tex.setAnchorPoint(x, y);
}

//----------------------------------------------------------
void goVideoGrabber::resetAnchor(){
   	if (bUseTexture)tex.resetAnchor();
}

//------------------------------------
void goVideoGrabber::draw(float _x, float _y, float _w, float _h){
	if (bUseTexture){
...
//------------------------------------
void goVideoGrabber::draw(float _x, float _y){
	draw(_x, _y, (float)width, (float)height);
}


//----------------------------------------------------------
float goVideoGrabber::getHeight(){
	return (float)height;
}

//----------------------------------------------------------
float goVideoGrabber::getWidth(){
	return (float)width;
}
